...
loadjs.ready("head", function () {
    ...
    fpacientesedit = currentForm = new ew.Form("fpacientesedit", "edit");
    ...
    fpacientesedit.addFields([
        ["id_paciente", [fields.id_paciente.visible && fields.id_paciente.required ? ew.Validators.required(fields.id_paciente.caption) : null], fields.id_paciente.isInvalid],
    ...
    // Validate form
    fpacientesedit.validate = function () {
    ...
    // Form_CustomValidate
    fpacientesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpacientesedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fpacientesedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpacientesedit" id="fpacientesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_paciente->Visible) { // id_paciente ?>
    <div id="r_id_paciente" class="form-group row">
        <label id="elh_pacientes_id_paciente" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_paciente->caption() ?><?= $Page->id_paciente->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_paciente->cellAttributes() ?>>
<span id="el_pacientes_id_paciente">
<span<?= $Page->id_paciente->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_paciente->getDisplayValue($Page->id_paciente->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pacientes" data-field="x_id_paciente" data-hidden="1" name="x_id_paciente" id="x_id_paciente" value="<?= HtmlEncode($Page->id_paciente->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
